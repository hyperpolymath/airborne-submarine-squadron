...
[arabic]
. Test obstacle spawning with correct probabilities
. Test collision detection for each obstacle type
. Test effect application (slowdown, pull, push, damage)
. Test destructible obstacles (iceberg health reduction)

==== Integration Tests

[arabic]
. Test full obstacle lifecycle (spawn → collision → effect →
destruction)
. Test multiple simultaneous obstacles
. Test obstacle interactions with power-ups
...
. Adjust spawn rates and effect strengths as needed

=== Balancing Considerations

* *Spawn Rate*: Start conservative (15-20% chance), adjust based on
difficulty
...
[arabic]
. Implement AffineScript types and functions
. Add obstacle spawning to game loop
. Implement collision detection and effects
. Update web interface with visuals
. Test and balance

This obstacle system will add environmental challenges and strategic
depth to the game while maintaining the proven-servers architecture and
