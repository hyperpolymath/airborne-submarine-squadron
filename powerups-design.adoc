...
[arabic]
. Test power-up spawning with correct probabilities
...
[arabic]
. Test full power-up lifecycle (spawn → collect → apply → expire)
. Test multiple simultaneous power-ups
. Test power-up interactions (e.g., shield + speed boost)
...
* *Rarity*: Common power-ups (health, ammo) vs rare (environment
control)
...
. Add power-up spawning to game loop
. Implement collision detection
. Add effect application logic
. Update web interface
. Test and balance

This power-up system will add significant depth and replayability to the
game while maintaining the proven-servers architecture and deterministic
