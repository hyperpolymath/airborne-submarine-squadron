...
// Power-up system (AffineScript core types)
enum PowerUpType {
  HealthPack,
  AmmoRefill,
  Shield,
...
// Obstacle system (AffineScript core types)
enum ObstacleType {
  Rock,
  Iceberg,
  CoralReef,
...
// Power-up initialization (AffineScript)
...
// Obstacle initialization (AffineScript)
...
fn maybe_spawn_powerup(world: World) -> World {
  ...
  if world.tick % 60 == 0 && rand_int(100) < 15 {
    if !world.powerups.a.active {
      return spawn_powerup_in_slot(world, "a");
    } else if !world.powerups.b.active {
      return spawn_powerup_in_slot(world, "b");
    } else {
      return spawn_powerup_in_slot(world, "c");
    };
  };
  return world;
}

fn spawn_powerup_in_slot(world: World, slot: String) -> World {
  let x = 850;
  let y = rand_int(500) + 50;
  let powerup = { x: x, y: y, powerup_type: PowerUpType::HealthPack, active: true, duration: 0, spawn_tick: world.tick };
...
fn maybe_spawn_obstacle(world: World) -> World {
  if world.obstacles.a.active && world.obstacles.b.active && world.obstacles.c.active && world.obstacles.d.active {
    return world;
  };
  
  if world.tick % 120 == 0 && rand_int(100) < 20 {
    if !world.obstacles.a.active {
      return spawn_obstacle_in_slot(world, "a");
    } else if !world.obstacles.b.active {
      return spawn_obstacle_in_slot(world, "b");
    } else if !world.obstacles.c.active {
      return spawn_obstacle_in_slot(world, "c");
    } else {
      return spawn_obstacle_in_slot(world, "d");
    };
  };
  return world;
}

fn spawn_obstacle_in_slot(world: World, slot: String) -> World {
  let x = 850;
  let y = rand_int(500) + 50;
  let width = 50;
  let height = 50;
  let obstacle_type = if is_air(world.env) {
    if rand_int(100) < 50 {
      ObstacleType::Rock;
    } else {
    ...
    if rand_int(100) < 30 {
      ...
      ObstacleType::CoralReef;
    } else if rand_int(100) < 80 {
      ObstacleType::Whirlpool;
    } else {
      ObstacleType::ThermalVent;
    };
  };
  
  let obstacle = { x: x, y: y, width: width, height: height, obstacle_type: obstacle_type, health: 100, active: true, effect_strength: 10 };
  ...
  if slot == "a" {
    return {
      ...
      active_powerups: world.active_powerups,
      obstacles: { a: obstacle, b: world.obstacles.b, c: world.obstacles.c, d: world.obstacles.d },
      active_effects: world.active_effects
    };
  } else if slot == "b" {
    return {
      ...
      powerups: world.powerups,
      active_powerups: world.active_powerups,
      obstacles: { a: world.obstacles.a, b: obstacle, c: world.obstacles.c, d: world.obstacles.d },
      active_effects: world.active_effects
    };
  } else if slot == "c" {
    return {
      ...
      powerups: world.powerups,
      active_powerups: world.active_powerups,
      obstacles: { a: world.obstacles.a, b: world.obstacles.b, c: obstacle, d: world.obstacles.d },
      active_effects: world.active_effects
    };
  } else {
    return {
      ...
      powerups: world.powerups,
      active_powerups: world.active_powerups,
      obstacles: { a: world.obstacles.a, b: world.obstacles.b, c: world.obstacles.c, d: obstacle },
      active_effects: world.active_effects
    };
  };
}
