...
fn spawn_in_powerup_slot(world: World, slot: String, tick: Int) -> World {
  ...
  let powerup_type =
    if rand < 20 {
      ...
      powerup_environment_control()
    };
  
  let duration =
    if powerup_type == powerup_health_pack() {
      0
    } else if powerup_type == powerup_ammo_refill() {
      0
    } else if powerup_type == powerup_shield() {
      300
    } else if powerup_type == powerup_speed_boost() {
      600
    } else if powerup_type == powerup_rapid_fire() {
      900
    } else if powerup_type == powerup_double_damage() {
      600
    } else {
      1200
    };
...
fn apply_powerup_effect(world: World, slot: String, powerup_type: PowerUpType) -> World {
  let new_powerups =
    if slot == "a" {
      { ...world.powerups, a: { ...world.powerups.a, active: false } }
    } else if slot == "b" {
      { ...world.powerups, b: { ...world.powerups.b, active: false } }
    } else {
      { ...world.powerups, c: { ...world.powerups.c, active: false } }
    };
...
fn update_powerup_timers(world: World) -> World {
  let shield =
    if world.active_powerups.shield {
      world.active_powerups.shield_timer - 1 > 0
    } else {
      false
    };
  let speed_boost =
    if world.active_powerups.speed_boost {
      world.active_powerups.speed_boost_timer - 1 > 0
    } else {
      false
    };
  let rapid_fire =
    if world.active_powerups.rapid_fire {
      world.active_powerups.rapid_fire_timer - 1 > 0
    } else {
      false
    };
  let double_damage =
    if world.active_powerups.double_damage {
      world.active_powerups.double_damage_timer - 1 > 0
    } else {
      false
    };
  let environment_control =
    if world.active_powerups.environment_control {
      world.active_powerups.environment_control_timer - 1 > 0
    } else {
      false
    };
  ...
  return { 
    ...
    active_powerups: { 
      ...
      shield_timer: if shield {
        world.active_powerups.shield_timer - 1
      } else {
        0
      },
      speed_boost_timer: if speed_boost {
        world.active_powerups.speed_boost_timer - 1
      } else {
        0
      },
      rapid_fire_timer: if rapid_fire {
        world.active_powerups.rapid_fire_timer - 1
      } else {
        0
      },
      double_damage_timer: if double_damage {
        world.active_powerups.double_damage_timer - 1
      } else {
        0
      },
      environment_control_timer: if environment_control {
        world.active_powerups.environment_control_timer - 1
      } else {
        0
      }
    }
...
fn deactivate_obstacle(world: World, slot: String, sub: Submarine) -> World {
  let new_obstacles =
    if slot == "a" {
      { ...world.obstacles, a: { ...world.obstacles.a, active: false } }
    } else if slot == "b" {
      { ...world.obstacles, b: { ...world.obstacles.b, active: false } }
    } else if slot == "c" {
      { ...world.obstacles, c: { ...world.obstacles.c, active: false } }
    } else {
      { ...world.obstacles, d: { ...world.obstacles.d, active: false } }
    };
...
fn update_obstacle(world: World, slot: String, obstacle: Obstacle, sub: Submarine) -> World {
  let new_obstacles =
    if slot == "a" {
      { ...world.obstacles, a: obstacle }
    } else if slot == "b" {
      { ...world.obstacles, b: obstacle }
    } else if slot == "c" {
      { ...world.obstacles, c: obstacle }
    } else {
      { ...world.obstacles, d: obstacle }
    };
  return { ...world, obstacles: new_obstacles, sub: sub };
}
