...
This document describes how the Airborne Submarine Squadron game
integrates with the proven-servers infrastructure.
...
[width="100%",cols="29%,35%,20%,16%",options="header",]
|===
|Game Concept |Proven PacketType |Direction |Purpose
|Initial state |`+Sync+` |Server→Client |Full game state synchronisation

|Player input |`+Input+` |Client→Server |Thrust, fire, toggle_env
commands

|State updates |`+StateUpdate+` |Server→Client |Deterministic state
after each tick
...
[arabic]
. *Client connects* → `+Connect+` packet
. *Server responds* → `+Sync+` packet with full state
. *Game loop*:
...
. *Disconnect* → `+Disconnect+` packet with reason
...
[arabic]
. Implement proven PacketType serialisation in WASM
. Add network layer to game client
. Implement server-side game loop with proven protocol
. Add multiplayer support and synchronisation
...
This integration follows proven-servers patterns: - Deterministic state
management - Explicit error handling - Type-safe protocols -
Safety-first design
